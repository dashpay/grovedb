...
impl Element {
    ...
    pub fn new_item_allowing_bidirectional_references(item_value: Vec<u8>) -> Self {
        Element::ItemWithBackwardsReferences(
            item_value,
            crate::bidirectional_reference::BackwardReferences::default(),
            None,
        )
    }
    ...
    /// Set element to an item without flags that can be targeted by up to
    /// `max_incoming` bidirectional references (at most
    /// [`crate::MAX_BACKWARD_REFERENCES`]; the declared capacity is what the
    /// batch cost estimator charges for a write of this element).
    pub fn new_item_allowing_bidirectional_references_with_capacity(
        item_value: Vec<u8>,
        max_incoming: u16,
    ) -> Self {
        Element::ItemWithBackwardsReferences(
            item_value,
            crate::bidirectional_reference::BackwardReferences::with_max_incoming(max_incoming),
            None,
        )
    }

    /// Set element to an item with flags that can be targeted by up to
    /// `max_incoming` bidirectional references.
    pub fn new_item_allowing_bidirectional_references_with_capacity_and_flags(
        item_value: Vec<u8>,
        max_incoming: u16,
        flags: Option<ElementFlags>,
    ) -> Self {
    ...
    /// Set element to a sum item without flags that can be targeted by up
    /// to `max_incoming` bidirectional references.
    pub fn new_sum_item_allowing_bidirectional_references_with_capacity(
        value: i64,
        max_incoming: u16,
    ) -> Self {
        Element::SumItemWithBackwardsReferences(
    ...
    /// Set element to a sum item with flags that can be targeted by up to
    /// `max_incoming` bidirectional references.
    pub fn new_sum_item_allowing_bidirectional_references_with_capacity_and_flags(
        value: i64,
        max_incoming: u16,
        flags: Option<ElementFlags>,
    ) -> Self {
    ...
    /// Set element to an item carrying an explicit sum value that can be
    /// targeted by up to `max_incoming` bidirectional references.
    pub fn new_item_with_sum_item_allowing_bidirectional_references_with_capacity(
        item_value: Vec<u8>,
        sum_value: i64,
        max_incoming: u16,
    ) -> Self {
    ...
    /// Set element to an item carrying an explicit sum value, with flags,
    /// that can be targeted by up to `max_incoming` bidirectional
    /// references.
    pub fn new_item_with_sum_item_allowing_bidirectional_references_with_capacity_and_flags(
        item_value: Vec<u8>,
        sum_value: i64,
        max_incoming: u16,
        flags: Option<ElementFlags>,
