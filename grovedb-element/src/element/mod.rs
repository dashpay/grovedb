...
pub enum Element {
    ...
    /// encoding on disk; `CountIndexedTree` is placed AFTER `NotSummed`
    /// (variant index 16 = NOT_SUMMED_WRAPPER_DISCRIMINANT) so this
    /// variant gets index 17, matching `ElementType::CountIndexedTree`.
    CountIndexedTree(
        Option<Vec<u8>>,
        Option<Vec<u8>>,
        CountValue,
...
mod cidx_tests {
    ...
    fn count_indexed_tree_display_renders_fields() {
        ...
        let with_keys =
            Element::CountIndexedTree(Some(vec![0xab, 0xcd]), Some(vec![0xef]), 7, None);
    ...
    fn provable_count_indexed_tree_display_renders_fields() {
        ...
        let populated = Element::ProvableCountIndexedTree(
            Some(vec![0x01]),
            Some(vec![0x02]),
            42,
            Some(vec![9]),
