...
mod tests {
    ...
    fn visualize_backward_references_family() {
        ...
        let item =
            Element::ItemWithBackwardsReferences(b"v".to_vec(), Default::default(), Some(vec![2]));
        let s = render(&item);
        assert!(
        ...
        let sum = Element::SumItemWithBackwardsReferences(-7, Default::default(), Some(vec![3]));
        let s = render(&sum);
        assert!(
            s.starts_with("sum_item_with_backwards_references: -7"),
            "got: {s}"
        );
        let sum_plain = Element::SumItemWithBackwardsReferences(-7, Default::default(), None);
        render(&sum_plain);
    }
