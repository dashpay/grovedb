//! `add_average_case_merk_propagate` v0 — grove v1 dispatch path
//! (historical baseline). Inherits the pre-existing buggy Mix divisor;
//! see `add_average_case_merk_propagate_v2` for the corrected formula.
//!
//! Uses `tree_type.inner_node_type()` for the Mix reference-cost
//! arms throughout (v1 changes the `storage_loaded_bytes` arm to
