//! Fuzz tests for AVL tree operations.
//!
//! These tests verify that the AVL tree maintains consistency with a reference
//! BTreeMap implementation across random insert, update, and delete operations.
...
mod tests {
    ...
    fn fuzz() {
        ...
        for _ in 0..ITERATIONS {
            ...
            fuzz_case(seed, false);
        }
    }

    #[test]
    fn fuzz_17391518417409062786() {
        fuzz_case(17391518417409062786, false);
    }

    #[test]
    fn fuzz_396148930387069749() {
        fuzz_case(396148930387069749, false);
    }
    ...
    fn fuzz_case(seed: u64, using_sum_trees: bool) {
        ...
        let tree_type = if using_sum_trees {
            TreeType::SumTree
        } else {
            TreeType::NormalTree
        };
        ...
        for j in 0..3 {
            ...
            println!("BATCH {j}");
            println!("{batch:?}");
            maybe_tree = apply_to_memonly(maybe_tree, &batch, tree_type, grove_version);
    ...
    fn make_batch(maybe_tree: Option<&TreeNode>, size: u64, seed: u64) -> Vec<BatchEntry<Vec<u8>>> {
        ...
        let insert = || (random_value(2), Op::Put(random_value(2), BasicMerkNode));
        let update = || {
            let key = get_random_key();
            (key.to_vec(), Op::Put(random_value(2), BasicMerkNode))
        };
        let delete = || {
            let key = get_random_key();
    ...
    fn apply_to_map(map: &mut Map, batch: &[BatchEntry<Vec<u8>>]) {
