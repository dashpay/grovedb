    ...
    S: Fetch + Sized + Clone,
{
    ...
    pub(crate) fn to_kv_value_hash_node(&self) -> Node {
        Node::KVValueHash(self.tree().key().to_vec(), self.tree().value().to_vec(), self.tree().value_hash().clone())
    }
...
mod test {
    ...
    fn doc_proof() {
        ...
        assert_eq!(
            bytes,
            vec![
                3, 1, 1, 0, 1, 1, 3, 1, 2, 0, 1, 2, 32, 3, 1, 3, 0, 1, 3, 3, 1, 4, 0, 1, 4, 32, 33,
                2, 61, 233, 169, 61, 231, 15, 78, 53, 219, 99, 131, 45, 44, 165, 68, 87, 7, 52,
                238, 68, 142, 211, 110, 161, 111, 220, 108, 11, 17, 31, 88, 197, 32, 1, 12, 156,
                232, 212, 220, 65, 226, 32, 91, 101, 248, 64, 225, 206, 63, 12, 153, 191, 183, 10,
                233, 251, 249, 76, 184, 200, 88, 57, 219, 2, 250, 113, 33
            ]
        );
