...
fn test_get_peak_map() {
    ...
    // 5 and 6 are not valid mmr_size, it will return the bitmap of the last valid
    // mmr (size 4)
    assert_eq!(get_peak_map(5), 0b11);
    assert_eq!(get_peak_map(6), 0b11);
    assert_eq!(get_peak_map(7), 0b100);
    assert_eq!(get_peak_map(8), 0b101);
    // 9 is not valid mmr_size, it will return the bitmap of the last valid mmr
    // (size 8)
...
fn test_get_peaks() {
    ...
    // 5 and 6 are not valid mmr_size, it will return the peaks of the last valid
    // mmr (size 4)
    assert_eq!(get_peaks(5), vec![2, 3]);
    assert_eq!(get_peaks(6), vec![2, 3]);
    assert_eq!(get_peaks(7), vec![6]);
    assert_eq!(get_peaks(8), vec![6, 7]);
    // 9 is not valid mmr_size, it will return the peaks of the last valid mmr (size
    // 8)
