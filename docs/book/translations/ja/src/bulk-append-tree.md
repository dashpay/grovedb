...
`AuxBulkStore` アダプタは GroveDB の `get_aux`/`put_aux`/`delete_aux` 呼び出しをラップし、コスト追跡のために `RefCell` 内に `OperationCost` を蓄積します。追記操作からのハッシュコストは `cost.hash_node_calls` に追加されます。
...
範囲 `[start, end)` の**生成ステップ**（`chunk_size = 1u32 << chunk_power`）：
...
**なぜすべてのバッファエントリを含めるのか？** バッファは密なマークル木であり、そのルートハッシュはすべてのエントリにコミットしています。検証者は `dense_tree_root` を検証するために、すべてのエントリからツリーを再構築する必要があります。バッファは `capacity`（最大65,535エントリ）で制限されているため、これは合理的なコストです。
...
検証が成功すると、`BulkAppendTreeProofResult` は検証済みのチャンクブロブとバッファエントリから特定の値を抽出する `values_in_range(start, end)` メソッドを提供します。
...
`state_root` は Merk 子ハッシュとして流れます（`insert_subtree` の `subtree_root_hash` パラメータ経由）。ステートルートの変更は GroveDB Merk 階層を通じてルートハッシュまで伝播します。

V1 証明（第9.6節）では、親 Merk 証明がエレメントバイトと子ハッシュバインディングを証明し、`BulkAppendTreeProof` がクエリされたデータが子ハッシュとして使用された `state_root` と整合していることを証明します。
...
| 証明生成 | チャンク数に依存 | チャンクあたりの密なマークルルート + MMR 証明 |
| 証明検証 | 2C·K - K + B·2 + 1 | Kチャンク、Bバッファエントリ、Cチャンクサイズ |
...
| `grovedb-bulk-append-tree/src/error.rs` | `BulkAppendError` 列挙型 |
