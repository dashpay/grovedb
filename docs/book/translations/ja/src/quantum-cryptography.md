...
| アルゴリズム | 対象 | 高速化 | 実際の影響 |
|------------|------|--------|----------|
| **Shor** | ECC 離散対数、RSA 因数分解 | 多項式時間（古典に対して指数的高速化） | ECC の**完全な破壊** |
...
### GroveDB インフラストラクチャ：現在の暗号学的仮定の下で量子安全と考えられる

GroveDB 自体のすべてのデータ構造は Blake3 ハッシュのみに依存しており、現在の
暗号学的仮定の下で量子耐性があると考えられています：
...
**既知の攻撃に基づく変更は不要です。** GroveDB の Merk ツリー証明、MMR 整合性
チェック、BulkAppendTree エポックルート、およびすべての V1 証明認証チェーンは
量子コンピュータに対して安全であると考えられています。ハッシュベースの
インフラストラクチャはポスト量子においてシステムの最も堅固な部分ですが、
新たな暗号解析技術の登場により評価が変わる可能性があります。
...
| ノート数 | 現在（312 B） | ハイブリッド（1,432 B） | 差分 |
|---------|-------------|---------------------|------|
...
| GroveDB Merk/MMR/Blake3 インフラストラクチャ | **現在の暗号学的仮定の下で既に量子安全。**既知の攻撃に基づく対応不要。 |
