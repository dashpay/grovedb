...
GroveDB のすべての操作は計算リソースで測定されたコストを蓄積します：
...
> **Sinsemilla ハッシュ呼び出し**は CommitmentTree アンカーのための楕円曲線ハッシュ操作を追跡します。これらは Blake3 ノードハッシュよりもはるかに高コストです。
...
すべての操作は結果を `CostContext` でラップして返します：
...
これにより**モナディック**なコスト追跡パターンが作られます — コストが操作の連鎖を通じて自動的に流れます：
...
## cost_return_on_error! マクロ

GroveDB コードで最も一般的なパターンは `cost_return_on_error!` マクロです。`?` のように動作しますが、早期リターン時にコストを保持します：
...
値が更新される場合、コストは新しい値がより大きいか、小さいか、同じサイズかによって異なります：
...
ハッシュコストは「ハッシュノード呼び出し」— Blake3 ブロック圧縮の回数で測定されます：

| 操作 | 入力サイズ | ハッシュ呼び出し数 |
|-----------|-----------|------------|
| `value_hash(small)` | 64バイト未満 | 1 |
| `value_hash(medium)` | 64-127バイト | 2 |
| `kv_hash` | key + value_hash | 可変 |
| `node_hash` | 96バイト（3 × 32） | 2（常に） |
| `combine_hash` | 64バイト（2 × 32） | 1（常に） |
| `node_hash_with_count` | 104バイト（3 × 32 + 8） | 2（常に） |
| Sinsemilla（CommitmentTree） | Pallas 曲線の EC 操作 | `sinsemilla_hash_calls` で個別追跡 |
...
GroveDB は操作を実行する前にコストを**見積もる**関数を提供します。これはブロックチェーンの手数料計算にとって重要です — 支払いを確約する前にコストを知る必要があります。
