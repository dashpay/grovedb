...
## GroveDB Alt Agac Oneki

```text
prefix = blake3(parent_prefix || key) → 32 bytes
```

## Durum Koku — BulkAppendTree

```text
state_root = blake3("bulk_state" || mmr_root || dense_tree_root)
```

Tampon bir DenseFixedSizedMerkleTree'dir — `dense_tree_root` onun kok hash'idir.

## Yogun Merkle Koku — BulkAppendTree Parcalari
...
## Yogun Agac Kok Hash'i — DenseAppendOnlyFixedSizeTree
...
## Yogun Agac Ispat Dogrulama — DenseAppendOnlyFixedSizeTree
...
## MMR Dugum Birlestirme — MmrTree / BulkAppendTree Parca MMR'si

```text
parent.hash = blake3(left.hash || right.hash)
```

## Sinsemilla Koku — CommitmentTree
...
*GroveDB Kitabi — geliştiriciler ve araştırmacilar icin GroveDB ic yapilarini belgeler. GroveDB kaynak koduna dayanmaktadir.*
