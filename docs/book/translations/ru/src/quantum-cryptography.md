...
### Инфраструктура GroveDB: считается квантово-безопасной при текущих предположениях

Все собственные структуры данных GroveDB основаны исключительно на хешировании Blake3,
которое считается квантово-устойчивым при текущих криптографических предположениях:
...
**На основании известных атак изменения не требуются.** Доказательства дерева Merk
в GroveDB, проверки согласованности MMR, корни эпох BulkAppendTree и все цепочки
аутентификации доказательств V1 считаются защищёнными от квантовых компьютеров.
Хеш-инфраструктура — самая стойкая часть системы в постквантовую эпоху, хотя оценки
могут измениться с появлением новых криптоаналитических методов.
