...
## Структура OperationCost

Каждая операция в GroveDB накапливает затраты, измеряемые в вычислительных ресурсах:
...
> **Вызовы хеширования Sinsemilla** отслеживают операции хеширования на эллиптических кривых для якорей CommitmentTree. Они значительно дороже хешей Blake3.

Затраты хранения детализируются далее:
...
Все операции возвращают результат, обёрнутый в `CostContext`:
...
Это создаёт **монадический** паттерн отслеживания затрат — затраты автоматически протекают через цепочки операций:
...
## Макрос cost_return_on_error!

Самый распространённый паттерн в коде GroveDB — макрос `cost_return_on_error!`, который действует как `?`, но сохраняет затраты при раннем возврате:
...
## Детализация затрат хранения

При обновлении значения затраты зависят от того, стало ли новое значение больше, меньше или осталось того же размера:

```mermaid
graph LR
    subgraph case1["СЛУЧАЙ 1: Тот же размер (old=100, new=100)"]
        c1_old["old: 100Б"]
        c1_new["new: 100Б"]
        c1_cost["replaced_bytes += 100"]
    end

    subgraph case2["СЛУЧАЙ 2: Увеличение (old=100, new=120)"]
        c2_old["old: 100Б"]
        c2_new["new: 120Б"]
        c2_replaced["заменено: 100Б"]
        c2_added["добавлено: +20Б"]
        c2_cost["replaced_bytes += 100<br/>added_bytes += 20"]
    end

    subgraph case3["СЛУЧАЙ 3: Уменьшение (old=100, new=70)"]
        c3_old["old: 100Б"]
        c3_new["new: 70Б"]
        c3_replaced["заменено: 70Б"]
        c3_removed["удалено: 30Б"]
...
## Затраты операций хеширования

Затраты хеширования измеряются в «вызовах хеширования узлов» — количестве блочных сжатий Blake3:

| Операция | Размер входа | Вызовы хеширования |
|-----------|-----------|------------|
| `value_hash(small)` | < 64 байт | 1 |
| `value_hash(medium)` | 64-127 байт | 2 |
| `kv_hash` | ключ + value_hash | зависит |
| `node_hash` | 96 байт (3 × 32) | 2 (всегда) |
| `combine_hash` | 64 байта (2 × 32) | 1 (всегда) |
| `node_hash_with_count` | 104 байта (3 × 32 + 8) | 2 (всегда) |
| Sinsemilla (CommitmentTree) | Операция на кривой Pallas | отслеживается отдельно через `sinsemilla_hash_calls` |

Общая формула для Blake3:

```text
hash_calls = 1 + (input_bytes - 1) / 64
```

## Оценка наихудшего и среднего случаев

GroveDB предоставляет функции для **оценки** затрат операций до их выполнения. Это критически важно для расчёта комиссий в блокчейне — нужно знать стоимость до того, как обязаться оплатить её.
