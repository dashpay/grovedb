...
При верификации доказательства из запроса с исключающим диапазоном вам может
понадобиться убедиться, что определённые ключи существуют как **граничные
элементы** — ключи, которые закрепляют диапазон, но не являются частью
...
доказывают существование пробела (см. [Доказательства отсутствия](#доказательства-отсутствия)
выше).

### Получение всех граничных ключей

После верификации доказательства вызовите `boundaries` на декодированном
`GroveDBProof`, чтобы получить все граничные ключи для данного пути:
...
Аргумент `path` определяет, какой уровень доказательства проверять
(соответствующий пути поддерева GroveDB, в котором был выполнен диапазонный
запрос).

### Проверка одного граничного ключа

Если вам нужно проверить только то, является ли один конкретный ключ граничным,
используйте `key_exists_as_boundary`:
...
- Если ключ курсора присутствует в `boundaries()`, курсор валиден — клиент
  может доверять тому, что пагинация привязана к реальному документу.
- Если он не присутствует, документ-курсор мог быть удалён между страницами,
  и клиенту следует рассмотреть перезапуск пагинации.

> **Важно:** И `boundaries()`, и `key_exists_as_boundary` выполняют
> синтаксический анализ узлов `KVDigest`/`KVDigestCount` доказательства. Они
> не предоставляют криптографических гарантий сами по себе — всегда сначала
> верифицируйте доказательство относительно доверенного корневого хеша. Те же
> типы узлов также встречаются в доказательствах отсутствия, поэтому вызывающий
> должен интерпретировать результаты в контексте запроса, породившего
> доказательство.

На уровне merk те же проверки доступны через
`boundaries_in_proof(proof_bytes)` и
`key_exists_as_boundary_in_proof(proof_bytes, key)` для работы непосредственно
с сырыми байтами доказательства merk.
