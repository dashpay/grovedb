...
Извлекает сохранённое значение (cmx || rho || payload) по его глобальной позиции:
...
### Формат хранимой записи (280 байт для DashMemo)
...
| Полная хранимая запись (cmx+rho+payload) | **280 байт** | **756 байт** | Запись BulkAppendTree |
...
Отдельные элементы (cmx || rho || payload) могут запрашиваться по позиции и доказываться с помощью
