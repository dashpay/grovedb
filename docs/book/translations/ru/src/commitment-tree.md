...
Извлекает сохранённое значение (cmx || rho || cv_net || payload) по его глобальной позиции:
...
### Формат хранимой записи (312 байт для DashMemo)

Каждая запись в BulkAppendTree содержит полную зашифрованную нотную запись. Полный
формат с учётом каждого байта:
...
Первые два поля (`cmx` и `rho`) — это **незашифрованные поля протокола** — они публичны
по дизайну. Оставшиеся три поля (`epk_bytes`, `enc_ciphertext`, `out_ciphertext`)
образуют `TransmittedNoteCiphertext` и являются зашифрованной полезной нагрузкой.

### Разбор полей

**cmx (32 байта)** — Извлечённый коммитмент ноты, элемент базового поля Pallas. Это
значение листа, добавляемое во фронтир Sinsemilla. Оно фиксирует все поля ноты
(получатель, сумма, случайность) без их раскрытия. cmx — это то, что делает ноту
«обнаруживаемой» в дереве коммитментов.

**rho (32 байта)** — Нуллификатор ноты, тратимой в данном действии. Нуллификаторы уже
публичны в блокчейне (они должны быть таковыми для предотвращения двойной траты).
Хранение `rho` рядом с коммитментом позволяет лёгким клиентам при пробной
дешифрации проверить `esk = PRF(rseed, rho)` и подтвердить `epk' == epk` без
отдельного поиска нуллификатора. Это поле располагается между `cmx` и шифротекстом
как незашифрованная привязка на уровне протокола.

**epk_bytes (32 байта)** — Эфемерный открытый ключ, сериализованная точка кривой Pallas.
Выводится детерминистически из `rseed` ноты через:
...
где `g_d = DiversifyHash(d)` — диверсифицированная базовая точка для диверсификатора
получателя. `epk` позволяет получателю вычислить общий секрет для дешифрации:
`shared_secret = [ivk] * epk`. Он передаётся открыто, так как ничего не раскрывает об
отправителе или получателе без знания `esk` или `ivk`.

**enc_ciphertext (104 байта для DashMemo)** — Зашифрованный открытый текст ноты,
полученный шифрованием ChaCha20-Poly1305 AEAD:
...
Симметричный ключ выводится через ECDH:
`key = BLAKE2b-256("Zcash_OrchardKDF", shared_secret || epk_bytes)`.

При дешифрации получателем (с использованием `ivk`) **открытый текст ноты** (88 байт для
DashMemo) содержит:

| Смещение | Размер | Поле | Описание |
|--------|------|-------|-------------|
| 0 | 1 | version | Всегда `0x02` (Orchard, пост-ZIP-212) |
| 1 | 11 | diversifier (d) | Диверсификатор получателя, выводит базовую точку `g_d` |
| 12 | 8 | value (v) | 64-битная сумма ноты little-endian в дафах |
| 20 | 32 | rseed | Случайное зерно для детерминистического вывода `rcm`, `psi`, `esk` |
| 52 | 36 | memo | Данные мемо уровня приложения (DashMemo: 36 байт) |
| **Итого** | **88** | | |

Первые 52 байта (version + diversifier + value + rseed) — это **компактная нота** —
лёгкие клиенты могут пробно дешифровать только эту часть, используя потоковый шифр
ChaCha20 (без проверки MAC), чтобы определить, принадлежит ли нота им. Если да, они
дешифруют полные 88 байт и проверяют MAC.

**out_ciphertext (80 байт)** — Зашифрованные исходящие данные, позволяющие **отправителю**
восстановить ноту постфактум. Зашифровано с помощью Outgoing Cipher Key:
...
При дешифрации отправителем (с использованием `ovk`) **исходящий открытый текст** (64 байта)
содержит:

| Смещение | Размер | Поле | Описание |
|--------|------|-------|-------------|
| 0 | 32 | pk_d | Диверсифицированный ключ передачи (открытый ключ получателя) |
| 32 | 32 | esk | Эфемерный секретный ключ (скаляр Pallas) |
| **Итого** | **64** | | |

С помощью `pk_d` и `esk` отправитель может восстановить общий секрет, дешифровать
`enc_ciphertext` и восстановить полную ноту. Если отправитель устанавливает `ovk = null`,
исходящий открытый текст заполняется случайными байтами перед шифрованием, делая
восстановление невозможным даже для отправителя (невосстановимый выход).

### Схема шифрования: ChaCha20-Poly1305

И `enc_ciphertext`, и `out_ciphertext` используют ChaCha20-Poly1305 AEAD (RFC 8439):

| Параметр | Значение |
|-----------|-------|
| Размер ключа | 256 бит (32 байта) |
| Nonce | `[0u8; 12]` (безопасно, так как каждый ключ используется ровно один раз) |
| AAD | Пустой |
| MAC-тег | 16 байт (Poly1305) |

Нулевой nonce безопасен, так как симметричный ключ выводится из нового обмена
Диффи-Хеллмана для каждой ноты — каждый ключ шифрует ровно одно сообщение.

### Сравнение размеров DashMemo и ZcashMemo

| Компонент | DashMemo | ZcashMemo | Примечания |
|-----------|----------|-----------|-------|
| Поле мемо | 36 байт | 512 байт | Данные приложения |
| Открытый текст ноты | 88 байт | 564 байта | 52 фиксированных + мемо |
| enc_ciphertext | 104 байта | 580 байт | открытый текст + 16 MAC |
| Полезная нагрузка шифротекста (epk+enc+out) | 216 байт | 692 байта | Передаётся на ноту |
| Полная хранимая запись (cmx+rho+cv_net+payload) | **312 байт** | **788 байт** | Запись BulkAppendTree |

Меньший мемо DashMemo (36 вместо 512 байт) уменьшает каждую хранимую запись на 476 байт —
существенно при хранении миллионов нот.

### Поток пробной дешифрации (лёгкий клиент)

Лёгкий клиент, сканирующий свои ноты, выполняет следующую последовательность для каждой
хранимой записи:
...
Шаг 5 объясняет, почему `rho` должен храниться рядом с шифротекстом — без него лёгкий
клиент не может проверить привязку эфемерного ключа при пробной дешифрации.
...
Отдельные элементы (cmx || rho || cv_net || payload) могут запрашиваться по позиции и доказываться с помощью
