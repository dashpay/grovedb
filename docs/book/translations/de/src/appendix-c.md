# Anhang C: Hash-Berechnungs-Referenz
...
## GroveDB-Teilbaum-Präfix

```text
prefix = blake3(parent_prefix || key) → 32 bytes
```

## Zustandswurzel — BulkAppendTree

```text
state_root = blake3("bulk_state" || mmr_root || dense_tree_root)
```

Der Puffer ist ein DenseFixedSizedMerkleTree — `dense_tree_root` ist sein Wurzel-Hash.

## Dichte Merkle-Wurzel — BulkAppendTree-Chunks
...
## Dichter-Baum-Wurzel-Hash — DenseAppendOnlyFixedSizeTree
...
## Dichter-Baum-Beweis-Verifikation — DenseAppendOnlyFixedSizeTree
...
## MMR-Knoten-Verschmelzung — MmrTree / BulkAppendTree-Chunk-MMR

```text
parent.hash = blake3(left.hash || right.hash)
```

## Sinsemilla-Wurzel — CommitmentTree
...
## combine_hash (Eltern-Kind-Bindung)
...
*Das GroveDB-Buch — Dokumentation der Interna von GroveDB für Entwickler und
Forscher. Basierend auf dem GroveDB-Quellcode.*
