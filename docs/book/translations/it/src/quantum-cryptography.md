...
correzione degli errori, il requisito reale è circa 4 milioni di qubit fisici.
...
Tuttavia, la profondità del circuito per l'algoritmo di Grover su uno spazio di
chiavi a 128 bit è ancora 2^64 operazioni quantistiche — molti crittografi ritengono
che questo non sarà mai pratico su hardware reale a causa dei limiti di decoerenza.
...
|-----------|-----------|---------------|-----------------|
| **Shor** | Logaritmo discreto ECC, fattorizzazione RSA | Tempo polinomiale (accelerazione esponenziale rispetto al classico) | **Rottura totale** di ECC |
...
ogni primitiva in base alla sua vulnerabilità quantistica:

### Vulnerabile al Quantistico (algoritmo di Shor — 0 bit post-quantistici)

| Primitiva | Dove è usata | Cosa si rompe |
|-----------|-------------|--------------|
...
| Primitiva | Dove è usata | Sicurezza post-quantistica |
|-----------|-------------|---------------------------|
...
| **ChaCha20-Poly1305** | Cifra enc_ciphertext e out_ciphertext (chiavi a 256 bit) | 128-bit ricerca chiave (sicuro, ma il percorso di derivazione della chiave tramite ECDH non lo è) |
| **PRF^expand** (BLAKE2b-512) | Deriva esk, rcm, psi da rseed | 128-bit sicurezza PRF |

### Infrastruttura di GroveDB: Ritenuta Sicura contro il Quantistico sotto le Assunzioni Attuali

Tutte le strutture dati proprie di GroveDB si basano esclusivamente sull'hashing Blake3,
ritenuto resistente al quantistico sotto le attuali assunzioni crittografiche:
...
**Nessuna modifica necessaria in base agli attacchi noti.** Le prove degli alberi Merk
di GroveDB, i controlli di coerenza MMR, le radici delle epoche di BulkAppendTree e
tutte le catene di autenticazione delle prove V1 sono ritenute sicure contro i computer
quantistici. L'infrastruttura basata su hash è la parte più solida del sistema
post-quantistico, anche se le valutazioni potrebbero evolversi con nuove tecniche
crittanalitiche.

## Minacce Retroattive vs. in Tempo Reale

Questa distinzione è fondamentale per stabilire le priorità su cosa correggere e quando.

**Le minacce retroattive** compromettono dati già archiviati. Un avversario registra
...
per nuove transazioni. Le vecchie transazioni sono già state validate e confermate
dalla rete.
...
Con `rseed` e `rho` (archiviati accanto al testo cifrato), l'avversario può calcolare
...
rivela **gli importi delle transazioni ma non le identità del mittente o del
destinatario**. L'avversario vede "qualcuno ha inviato 5.0 Dash a qualcuno" ma non
può collegare l'importo a nessun indirizzo o identità senza rompere anche la
cifratura delle note.

Di per sé, gli importi senza collegamento hanno utilità limitata. Ma combinati con
...
Questa è la minaccia quantistica più urgente e pratica.

**Modello di attacco:** Un avversario statale (o qualsiasi parte con sufficiente
capacità di archiviazione) registra tutti i dati delle transazioni schermate on-chain
...
Passo 4: Derivare la chiave simmetrica (BLAKE2b è sicuro quantisticamente, ma l'input è compromesso):
...
**Punto chiave:** La cifratura simmetrica (ChaCha20-Poly1305) è perfettamente sicura
contro il quantistico. La vulnerabilità risiede interamente nel **percorso di
derivazione della chiave** — la chiave simmetrica è derivata da un segreto condiviso
ECDH, e l'ECDH è rotto dall'algoritmo di Shor. L'attaccante non rompe la cifratura;
recupera la chiave.

**Retroattività:** Questo attacco è **completamente retroattivo**. Ogni nota cifrata
mai archiviata sulla blockchain può essere decifrata una volta che esiste un CRQC.
I dati non possono essere ri-cifrati o protetti a posteriori. Ecco perché deve essere
...
ML-KEM è il meccanismo di incapsulamento di chiavi post-quantistico standardizzato
...
**ML-KEM-768** è la scelta raccomandata — è il set di parametri utilizzato da X-Wing,
...
Il KEM combinato è sicuro IND-CCA2 se **uno qualsiasi** dei KEM componenti è sicuro.
Questo è formalmente dimostrato da [Giacon, Heuer e Poettering (2018)](https://eprint.iacr.org/2018/024)
...
```text
┌──────────────────────────────────────────────────────────────────┐
│  Attuale (312 bytes)           Ibrido (1,432 bytes)              │
│                                                                  │
│  cmx:             32         cmx:              32               │
│  rho:             32         rho:              32               │
│  cv_net:          32         cv_net:           32               │
...
| Note | Attuale (312 B) | Ibrido (1,432 B) | Delta |
|------|----------------|------------------|-------|
...
La chiave pubblica ML-KEM di 1,184 byte deve essere inclusa nell'indirizzo affinché
i mittenti possano eseguire l'incapsulamento. Con circa 1,960 caratteri Bech32m, è
grande ma rientra ancora in un codice QR (massimo ~2,953 caratteri alfanumerici).

### Gestione delle Chiavi

La coppia di chiavi ML-KEM è derivata deterministicamente dalla chiave di spesa:
...
la chiave ML-KEM perché è derivata deterministicamente dalla chiave di spesa. Il
recupero del portafoglio funziona come prima.

**Gli indirizzi diversificati** condividono tutti lo stesso `ek_pq` perché ML-KEM non
ha un meccanismo di diversificazione naturale come la moltiplicazione scalare di
Pallas. Questo significa che un osservatore con due indirizzi di un utente può
...
Scansione di 100,000 note: ~10.1 sec → ~14.1 sec. L'overhead è significativo ma non
proibitivo. Il decapsulamento ML-KEM è in tempo costante senza vantaggio di
elaborazione in batch (a differenza delle operazioni su curve ellittiche), quindi scala
linearmente.

### Impatto sui Circuiti ZK

**Nessuno.** Il KEM ibrido è interamente nel livello di trasporto/cifratura. Il
...
| **Zcash** | Bozza ZIP per recuperabilità quantistica (recupero fondi, non cifratura) | Solo discussione |
...
- **Stato attuale (2026):** Nessun computer quantistico può rompere ECC a 255 bit.
  La più grande fattorizzazione quantistica dimostrata: ~50 bit. Divario: ordini di
  grandezza.
...
**1. Progettare per l'aggiornabilità ora.** Assicurarsi che il formato del record
...
transazioni schermate — gli utenti che si nascondono in un gruppo più piccolo hanno
meno privacy, non di più. Quando distribuito, ogni nota utilizza lo schema ibrido.

**3. Puntare alla finestra 2028-2030.** Questo è ben prima di qualsiasi minaccia
...
| Componente | Perché può aspettare |
|------------|---------------------|
| Firme di autorizzazione di spesa | La falsificazione è in tempo reale, non retroattiva. Aggiornare a ML-DSA/SLH-DSA prima dell'arrivo del CRQC. |
| Sistema di prove Halo 2 | La falsificazione di prove è in tempo reale. Migrare a un sistema basato su STARK quando necessario. |
...
| ML-KEM-512 | Lattice (MLWE) | 800 B | 768 B | 1 (128-bit) | Più piccolo, margine inferiore |
...
|---------|---------|-----------------|-----------------|------|
| STARKs | Funzioni hash (resistenza alle collisioni) | ~100-400 KB | **Sì** | Usato da StarkNet |
| Plonky3 | FRI (impegno polinomiale basato su hash) | ~50-200 KB | **Sì** | Sviluppo attivo |
| Halo 2 (attuale) | ECDLP sulle curve Pasta | ~5 KB | **No** | Sistema attuale di Orchard |
| Lattice SNARKs | MLWE | Ricerca | **Sì** | Non pronto per la produzione |

### Ecosistema di Crate Rust

| Crate | Fonte | FIPS 203 | Verificato | Note |
|-------|-------|----------|------------|------|
| `libcrux-ml-kem` | Cryspen | Sì | Formalmente verificato (hax/F*) | Massima garanzia |
| `ml-kem` | RustCrypto | Sì | Tempo costante, non verificato | Compatibilità con l'ecosistema |
| `fips203` | integritychain | Sì | Tempo costante | Rust puro, no_std |
...
│    2026-2028: Progettare per aggiornabilità, versionare i formati  │
