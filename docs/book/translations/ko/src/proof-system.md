...
배타적 범위 쿼리의 증명을 검증할 때, 특정 키가 **경계 엘리먼트**로 존재하는지 확인해야 할 수 있습니다 — 경계 엘리먼트는 범위를 고정하지만 결과 집합에는 포함되지 않는 키입니다.

예를 들어, `RangeAfter(10)` (10 이후의 모든 키)의 경우, 증명은 키 10을 `KVDigest` 노드로 포함합니다. 이는 키 10이 트리에 존재하며 범위의 시작을 고정함을 증명하지만, 키 10은 결과에 반환되지 않습니다.

### 경계 노드가 나타나는 경우

경계 `KVDigest` (또는 ProvableCountTree의 경우 `KVDigestCount`) 노드는 배타적 범위 쿼리 타입의 증명에서 나타납니다:
...
경계 노드는 부재 증명에서도 나타나며, 인접 키가 갭의 존재를 증명합니다 (위의 [부재 증명](#부재-증명) 참조).

### 모든 경계 키 조회

증명을 검증한 후, 디코딩된 `GroveDBProof`에서 `boundaries`를 호출하여 주어진 경로의 모든 경계 키를 가져올 수 있습니다:
...
`path` 인수는 증명의 어느 레이어를 검사할지 식별합니다 (범위 쿼리가 실행된 GroveDB 서브트리 경로와 일치).

### 단일 경계 키 확인

특정 키 하나가 경계인지만 확인하면 되는 경우 `key_exists_as_boundary`를 사용합니다:
...
이것은 **페이지네이션**에 특히 유용합니다. 클라이언트가 "문서 X 이후의 다음 100개 문서"를 요청할 때, 쿼리는 `RangeAfter(document_X_id)`입니다. 증명은 문서 101-200을 반환하지만, 클라이언트는 문서 X (페이지네이션 커서)가 트리에 여전히 존재하는지 확인하고 싶을 수 있습니다:

- 커서 키가 `boundaries()`에 나타나면, 커서가 유효합니다 — 클라이언트는 페이지네이션이 실제 문서에 고정되어 있음을 신뢰할 수 있습니다.
- 나타나지 않으면, 커서 문서가 페이지 사이에 삭제되었을 수 있으며, 클라이언트는 페이지네이션을 다시 시작하는 것을 고려해야 합니다.

> **중요:** `boundaries()`와 `key_exists_as_boundary` 모두 증명의 `KVDigest`/`KVDigestCount` 노드에 대한 구문적 스캔을 수행합니다. 이것만으로는 암호학적 보증을 제공하지 않습니다 — 항상 신뢰할 수 있는 루트 해시에 대해 먼저 증명을 검증하십시오. 동일한 노드 타입이 부재 증명에서도 나타나므로, 호출자는 증명을 생성한 쿼리의 맥락에서 결과를 해석해야 합니다.

Merk 레벨에서는 원시 merk 증명 바이트로 직접 작업하기 위해 `boundaries_in_proof(proof_bytes)` 및 `key_exists_as_boundary_in_proof(proof_bytes, key)`를 통해 동일한 검사를 사용할 수 있습니다.
