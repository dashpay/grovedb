...
## بادئة شجرة GroveDB الفرعية

```text
prefix = blake3(parent_prefix || key) → 32 bytes
```

## جذر الحالة — BulkAppendTree

```text
state_root = blake3("bulk_state" || mmr_root || dense_tree_root)
```

المخزن المؤقت هو DenseFixedSizedMerkleTree — و`dense_tree_root` هو تجزئة جذره.

## جذر ميركل الكثيف — شرائح BulkAppendTree
...
## تجزئة جذر الشجرة الكثيفة — DenseAppendOnlyFixedSizeTree
...
## التحقق من برهان الشجرة الكثيفة — DenseAppendOnlyFixedSizeTree
...
## دمج عقد MMR — MmrTree / نطاق MMR لشرائح BulkAppendTree

```text
parent.hash = blake3(left.hash || right.hash)
```

## جذر Sinsemilla — CommitmentTree
...
*كتاب GroveDB — توثيق البنية الداخلية لـ GroveDB للمطورين والباحثين.
مبنيّ على الشيفرة المصدرية لـ GroveDB.*
