...
من أن مفتاحاً معيناً موجود كـ**عنصر حدّي** — مفتاح يُرسي
النطاق لكنه ليس جزءاً من مجموعة النتائج.

على سبيل المثال، مع `RangeAfter(10)` (جميع المفاتيح بعد 10 بشكل صارم)، يتضمن البرهان
المفتاح 10 كعقدة `KVDigest`. هذا يُثبت أن المفتاح 10 موجود في الشجرة
ويُرسي بداية النطاق، لكن المفتاح 10 لا يُعاد في النتائج.

### متى تظهر عقد الحدود

تظهر عقد `KVDigest` الحدّية (أو `KVDigestCount` لـ ProvableCountTree) في
البراهين لأنواع استعلامات النطاق الحصرية:

| نوع الاستعلام | مفتاح الحدود | ما يُثبته |
|------------|-------------|----------------|
...
### التحقق من مفاتيح الحدود

بعد التحقق من برهان، يمكنك التحقق مما إذا كان مفتاح موجوداً كعنصر
حدّي باستخدام `key_exists_as_boundary` على `GroveDBProof` المُفكّك:
...
وسيط `path` يُحدّد أي طبقة من البرهان سيتم فحصها (مطابقة لمسار
شجرة GroveDB الفرعية حيث تم تنفيذ استعلام النطاق)، و`key` هو
مفتاح الحدود المراد البحث عنه.

### الاستخدام العملي: التحقق من التقسيم إلى صفحات

هذا مفيد بشكل خاص لـ**التقسيم إلى صفحات**. عندما يطلب عميل "المستندات
الـ 100 التالية بعد المستند X"، يكون الاستعلام `RangeAfter(document_X_id)`. يُعيد
البرهان المستندات 101–200، لكن قد يرغب العميل أيضاً في التأكد من أن
المستند X (مؤشر التقسيم إلى صفحات) لا يزال موجوداً في الشجرة:

- إذا أعادت `key_exists_as_boundary` القيمة `true`، فالمؤشر صالح — يمكن للعميل
  الوثوق بأن التقسيم إلى صفحات مُرسى بمستند حقيقي.
- إذا أعادت `false`، فقد يكون مستند المؤشر قد حُذف بين
  الصفحات، ويجب على العميل التفكير في إعادة بدء التقسيم إلى صفحات.

> **مهم:** تقوم `key_exists_as_boundary` بمسح بنيوي لعقد
> `KVDigest`/`KVDigestCount` في البرهان. لا توفر ضماناً تشفيرياً
> بذاتها — تحقق دائماً من البرهان مقابل تجزئة جذر موثوقة أولاً.
> تظهر أنواع العقد نفسها أيضاً في براهين الغياب، لذا يجب على المستدعي
> تفسير النتيجة في سياق الاستعلام الذي أنتج البرهان.

على مستوى merk، نفس التحقق متاح عبر
