...
|-----------|---------|---------|------------------|
| **Shor** | ECC اللوغاريتم المتقطع، تحليل RSA | زمن متعدد الحدود (تسريع أسي مقارنة بالكلاسيكي) | **كسر كامل** لـ ECC |
...
|--------|-----------|-----------|-------------|-------|
| STARKs | دوال التجزئة (مقاومة التصادم) | ~100-400 KB | **نعم** | مستخدم في StarkNet |
| Plonky3 | FRI (التزام متعدد الحدود قائم على التجزئة) | ~50-200 KB | **نعم** | تطوير نشط |
| Halo 2 (الحالي) | ECDLP على منحنيات Pasta | ~5 KB | **لا** | نظام Orchard الحالي |
| Lattice SNARKs | MLWE | بحثي | **نعم** | غير جاهز للإنتاج |

### نظام حزم Rust البيئي

| الحزمة | المصدر | FIPS 203 | مُتحقق | ملاحظات |
|-------|--------|----------|----------|-------|
| `libcrux-ml-kem` | Cryspen | نعم | مُتحقق رسمياً (hax/F*) | أعلى ضمان |
| `ml-kem` | RustCrypto | نعم | زمن ثابت، غير مُراجع | توافق النظام البيئي |
| `fips203` | integritychain | نعم | زمن ثابت | Rust نقي، no_std |
