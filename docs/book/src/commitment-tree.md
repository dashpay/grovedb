...
Step 3: Open data storage context at ct_path
        ...
        (cv_net never enters the frontier — the anchor is unchanged)
...
Retrieves a stored value (cmx || rho || cv_net || payload) by its global position:
...
### Stored Record Layout (312 bytes for DashMemo)
...
The first three fields (`cmx`, `rho`, and `cv_net`) are **unencrypted protocol
fields** — they are public by design. The remaining three fields (`epk_bytes`,
`enc_ciphertext`, `out_ciphertext`) form the `TransmittedNoteCiphertext` and
are the encrypted payload. Because `cv_net` is a fixed 32-byte field inserted
between `rho` and the payload, the ciphertext still deserializes from the
record's payload slice (offset 96 onward) exactly as before — only its starting
offset shifts from 64 to 96.
...
**cv_net (32 bytes)** — The note's net value commitment, a serialized Pallas
curve point (`cv_net = [v]·V + [rcv]·R`). Like `rho`, it is stored as an
unencrypted protocol field — it sits between `rho` and the ciphertext payload
and never enters the Sinsemilla frontier, so the Orchard anchor is unaffected.
It is stored because it is **required for outgoing-note (OVK) recovery**: the
sender's Outgoing Cipher Key is derived as
`ock = BLAKE2b-256("Zcash_Orchardock", ovk || cv_net || cmx || epk)` (see
`out_ciphertext` below), and `cv_net` cannot be recomputed from the note alone.
Without it stored, a light wallet holding only the `ovk` could not reconstruct
`ock` to decrypt `out_ciphertext` and recover a note it sent — Platform does not
retain transitions, so the value commitment would otherwise be lost.
...
The `cv_net` input to `ock` is exactly why the value commitment is stored as an
unencrypted field in the record (offset 64): without it, a wallet holding only
the `ovk` could not reconstruct `ock` and would be unable to decrypt
`out_ciphertext` to recover a note it sent.
...
1. Read record: cmx (32) || rho (32) || cv_net (32) || epk (32) || enc_ciphertext (104) || out_ciphertext (80)
   (incoming-note scanning ignores cv_net; it is consumed by the outgoing/OVK path)
...
Individual items (cmx || rho || cv_net || payload) can be queried by position and proved using
