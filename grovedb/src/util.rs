...
//     (&mut $cost:ident, $db:expr, $path:expr, $transaction:ident,
// $storage:ident, $root_key:ident, { $($body:tt)* }) => {         {
//             use ::storage::Storage;
// 	    let mut path = $path;
//             let $storage = $db
//                 .get_transactional_storage_context(path.clone(),
// $transaction).unwrap_add_cost(&mut $cost);             if let Some(last) =
// path.next_back() {                 let parent_storage =
// $db.get_transactional_storage_context(path,
// $transaction).unwrap_add_cost(&mut $cost);                 let element =
// cost_return_on_error!(                     &mut $cost,
// 		    Element::get_from_storage(&parent_storage, last).map_err(|e| {
//                         Error::CorruptedData(
//                             format!("could not get key for parent of subtree
// on tx: {}", e)                         )
//                     })
// 		);
//                 if let Element::Tree(root_key, _) = element {
...
//     (&mut $cost:ident, $db:expr, $path:expr, $storage:ident, $root_key:ident,
// { $($body:tt)* }) => {         {
//             use ::storage::Storage;
// 	    let mut path = $path;
//             let $storage = $db
...
//             storage_context_with_parent_no_tx!(&mut $cost, $db, $path,
// storage, root_key, {                 #[allow(unused_mut)]
...
//             storage_context_with_parent_using_tx!(&mut $cost, $db, $path,
// $transaction, storage, root_key, {                 #[allow(unused_mut)]
//                 let mut $subtree = cost_return_on_error!(
//                     &mut $cost,
//                     ::merk::Merk::open_with_root_key(storage, root_key)
//                         .map(|merk_res|
