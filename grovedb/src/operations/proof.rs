...
impl GroveDb {
        ...
        for (key, value_bytes) in subtree.get_kv_pairs(query.query.query.left_to_right).iter() {
            ...
            match element {
                Element::Tree(tree_hash) => {
                    if tree_hash == EMPTY_TREE_HASH { continue; }
                    ...
                    if self.check_subtree_exists_path_not_found(new_path.clone(), None, None).is_err() {
                        continue
                    }
    ...
    fn open_subtree(&self, path: &Vec<&[u8]>) -> Result<Merk<PrefixedRocksDbStorageContext>, Error> {
    ...
    fn prove_path(&self, mut proof_result: &mut Vec<u8>, path_slices: Vec<&[u8]>) -> Result<(), Error>{
