...
/// Classification of an `AggregateSumOnRange` `PathQuery`. Encodes
...
pub(super) struct AggregateSumClassification {
    /// The inner range that the leaf merk sum proof must satisfy.
...
/// Classify an `AggregateSumOnRange` path query and validate it at
/// the PathQuery level. The shape-specific pagination rules are
/// enforced through [`PathQuery::validate_aggregate_sum_on_range`]:
...
pub(super) fn classify_aggregate_sum_path_query(
    ...
    let leaf_inner = path_query.validate_aggregate_sum_on_range()?.clone();
    let q = &path_query.query.query;
    if q.aggregate_sum_on_range().is_some() {
        // Leaf shape: top-level `AggregateSumOnRange` item. The
        // top-level `validate_aggregate_sum_on_range` dispatcher above
        ...
        return Ok(AggregateSumClassification {
