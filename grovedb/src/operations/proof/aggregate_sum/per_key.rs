...
//! the [`AggregateSumClassification`], walks the path-prefix layers
//! with single-key descents, then either emits a single
//! `(empty_key, i64)` entry (leaf shape) or executes the carrier's
//! multi-key merk proof and recurses through `subquery_path` per
//! matched outer key (carrier shape).
//!
//! Sum-side mirror of
//! [`crate::operations::proof::aggregate_count::per_key`]. V0
//! (`MerkOnlyLayerProof`) envelopes are rejected at the entry-point
    ...
    operations::proof::{
        aggregate_sum::{
            ...
            helpers::{
                ...
                verify_single_key_layer_proof_v0, verify_sum_leaf, OuterMatch,
            },
        },
        LayerProof,
    },
...
fn verify_v1_per_key(
    ...
    if depth < path_keys.len() {
        ...
        // element is itself the ProvableSumTree /
        // ProvableCountProvableSumTree. In carrier shape the final path
        // element is the carrier (which could be any tree containing
        // sum-bearing trees as children); the terminal type check is
...
/// leaf sum proof, enforcing the chain at each step. Returns one
...
fn verify_v1_carrier_layer(
    ...
    let subquery_path = classification
        ...
        .ok_or_else(|| {
            Error::InvalidProof(
                path_query.clone(),
                "carrier aggregate-sum classification missing subquery_path".to_string(),
            )
        })?;
    ...
    } in matched
    {
        let lower_layer = layer.lower_layers.get(&outer_key).ok_or_else(|| {
            Error::InvalidProof(
                path_query.clone(),
                format!(
                    "carrier aggregate-sum proof missing lower layer for outer key {}",
                    hex::encode(&outer_key)
                ),
            )
        })?;

        let (lower_root, sum) = verify_v1_subquery_path(
            lower_layer,
            path_query,
            subquery_path,
            0,
...
/// terminating in the merk-level sum verifier.
fn verify_v1_subquery_path(
    ...
    let lower_layer = layer.lower_layers.get(&next_key).ok_or_else(|| {
        Error::InvalidProof(
            path_query.clone(),
            format!(
                "carrier aggregate-sum proof missing subquery_path layer for key {}",
                hex::encode(&next_key)
            ),
        )
    })?;
    let (lower_hash, sum) = verify_v1_subquery_path(
        lower_layer,
        path_query,
        subquery_path,
        depth + 1,
