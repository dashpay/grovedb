//! Combined-aggregate-specific helpers for the dual-axis PCPS host.
//! Items shared with the count and sum axes (the multi-key outer
//! walker, the single-key descent, the merk-bytes unwrapper, the
//! `OuterMatch` row type) live in
...
//!   The axis-specific terminal-type set is why this helper stays
//!   per-axis.

use grovedb_merk::{
    proofs::query::aggregate_count_and_sum::verify_aggregate_count_and_sum_on_range_proof,
...
/// Combined-aggregate axis label used by the shared diagnostic-prefix
/// helpers in `aggregate_common`.
const AXIS_LABEL: &str = "combined-aggregate";

/// Thin wrapper around [`super::super::aggregate_common::expect_merk_bytes`]
/// that supplies the combined-aggregate axis label.
...
/// supplies the combined-aggregate axis label.
