...
impl GroveDb {
    ...
    pub(crate) fn delete_internal_on_transaction_v3<B: AsRef<[u8]>>(
        &self,
        path: SubtreePath<B>,
        key: &[u8],
        ...
        if options.backward_references_policy.maintains() {
            self.delete_with_backward_references_v3(
                path,
                key,
                options,
                transaction,
    ...
    #[allow(clippy::too_many_arguments)]
    fn delete_with_backward_references_v3<B: AsRef<[u8]>>(
        &self,
        path: SubtreePath<B>,
        key: &[u8],
        options: &DeleteOptions,
