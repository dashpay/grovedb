//! `delete_operation_for_delete_internal` version 0.
//!
//! Differs from [v1](super::v1) ONLY in which pending deletes at the tree's
//! own path count as removing a key: every delete does, a `DeleteTree` with
//! `SubelementsDeletionBehavior::Skip` included. The apply path drops such a
//! `Skip` delete when its tree is not empty, so a parent emptied only by it
//! is deleted with `DontCheckWithNoCleanup` while the child is still in it,
//! orphaning the child and its contents.
//!
//! Selected by `GROVE_V1`, `GROVE_V2` and `GROVE_V3` — all live in
//! production, so the rule is kept bug-for-bug for replay compatibility.
...
impl GroveDb {
    /// Build the delete operation for one element — version 0 (see the
    /// module documentation).
    #[allow(clippy::too_many_arguments)]
    pub(crate) fn delete_operation_for_delete_internal_v0<
        'a,
        B: AsRef<[u8]>,
    ...
    >(
        ...
        if path.is_root() {
            ...
            if let MaybeTree::Tree(tree_type) = tree_type {
                ...
                // Every delete counts, a `DeleteTree` with
                // `SubelementsDeletionBehavior::Skip` included.
                    ...
                    .filter(|op| op.path.eq_path_vec(&subtree_merk_path_vec))
                {
                    match op.op {
                        ...
                        | GroveOp::DeleteTreeDontCheckForBackwardsReferences(..) => {
                            if merk_backed && let Some(deleted_key) = op.key.as_ref() {
                ...
                // The child the up-tree chain deletes at the level below.
