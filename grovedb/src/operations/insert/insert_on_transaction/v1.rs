...
//! [`InsertOptions::propagate_backward_references_when_unsure`] runs the exact shipped
...
//! aggregation wrappers are rejected when `propagate_backward_references_when_unsure`
...
fn insert_with_backward_references<'db, 'b, B: AsRef<[u8]>>(
    ...
    match element {
        ...
        | Element::NotCountedOrSummed(..) => {
            ...
            // flag.
            return Err(Error::NotSupported(
                "this element type cannot be inserted with propagate_backward_references_when_unsure set; \
