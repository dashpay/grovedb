...
use grovedb_merk::proofs::query::{Key, QueryItem};
...
use crate::query_result_type::{BTreeMapLevelResult, BTreeMapLevelResultOrItem, QueryResultType};
...
impl GroveDb {
    ...
    /// If we have more than one path query, we merge into a single path query before
    /// proving.
    ...
    fn prove_internal_serialized(&self, path_query: &PathQuery, prove_options: Option<ProveOptions>) -> CostResult<Vec<u8>, Error> {
        let mut cost = OperationCost::default();
        let proof = cost_return_on_error!(&mut cost, self.prove_internal(path_query, prove_options));
        let config = bincode::config::standard()
            .with_big_endian()
            .with_no_limit();
        let encoded_proof = cost_return_on_error_no_add!(&cost, bincode::encode_to_vec(proof, config).map_err(|e| Error::CorruptedData(format!("unable to encode proof {}", e))));
        Ok(encoded_proof).wrap_with_cost(cost)
    }

    /// Generates a proof
    fn prove_internal(&self, path_query: &PathQuery, prove_options: Option<ProveOptions>) -> CostResult<GroveDBProof, Error> {
        let ProveOptions {
        ...
        // we want to query raw because we want the references to not be resolved at this point

        let precomputed_result_map = cost_return_on_error!(
                &mut cost,
                self.query_raw(path_query, false, true, false, QueryResultType::QueryPathKeyElementTrioResultType, None)).0.to_btree_map_level_results();
    ...
    fn prove_subqueries(
        ...
        let (query_at_path, left_to_right) = cost_return_on_error_no_add!(
            &cost,
            path_query.query_items_at_path(path.as_slice()).ok_or(Error::CorruptedPath("path should be part of path_query")));
        ...
        let merk_proof = cost_return_on_error!(&mut cost, self.generate_merk_proof(
            &path.as_slice().into(),
            &subtree,
            &query_at_path,
            left_to_right,
        ...
        let lower_layers = cost_return_on_error_no_add!(
            &cost,
            layer_precomputed_results.key_values.into_iter().filter_map(|(key, value)| {
            match value {
                BTreeMapLevelResultOrItem::BTreeMapLevelResult(layer) => {
                    let mut lower_path = path.clone();
                    lower_path.push(key.as_slice());
                    match self.prove_subqueries(lower_path, path_query, layer).unwrap_add_cost(&mut cost) {
                        Ok(layer_proof) => {
                            Some(Ok((key, layer_proof)))
                        }
                        Err(e) => { Some(Err(e))}
                    }
                }
        ...
        B: AsRef<[u8]>,
    {
        ...
        let mut proof_result = cost_return_on_error_no_add!(
            &cost,
            subtree
                .prove_unchecked(query_items.clone(), limit, left_to_right)
                .unwrap()
                .map_err(|_e| Error::InternalError("failed to generate proof"))
        );

        cost_return_on_error!(&mut cost, self.post_process_merk_proof(path, &mut proof_result));

        let mut proof_bytes = Vec::with_capacity(128);
        encode_into(proof_result.iter(), &mut proof_bytes);
