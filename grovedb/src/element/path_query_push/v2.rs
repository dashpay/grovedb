//! `path_query_push` — **v2** (`GROVE_V4`+).
//!
//! The per-instance-limit engine. Three changes over [`super::v1`]:
//!
...
//!    global budget is settled from that. v0/v1 subtract returned rows
//!    only, silently dropping empty-subtree charges made below the
//!    first nesting level; the prover threads one shared counter and
//!    keeps them, so v2 aligns the read path with proof accounting.
...
//! 3. **The empty-subtree charge fires on zero consumption.** v1
//!    charges when the child returned no rows and skipped nothing; v2
//!    charges when the child *consumed* nothing and skipped nothing —
//!    a child that burned budget on its own empty subtrees already
//!    moved the counters, which is exactly the prover's
//!    "has a result at this level" test. v1's issue-#690 guard (offset
//!    skips are pagination, not absence) is carried over.
...
/// `path_query_push` v2 — see the module documentation.
pub(crate) fn path_query_push_v2(
    ...
    if element.is_any_tree() {
        ...
        if let Some(subquery) = subquery {
            ...
            // (v1's issue-#690 guard) eats a slot here.
            ...
            if budget.offset.unwrap_or(0) == 0 {
                ...
                if let Some(element) = maybe_element {
                    ...
                    // unconditionally here; v2 keeps the global charge
