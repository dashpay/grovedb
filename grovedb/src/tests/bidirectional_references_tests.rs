...
#[test]
fn specialized_types_and_wrappers_work_with_automatic_maintenance() {
    ...
    .unwrap();
    for (key, element) in [
        (b"mmr".as_slice(), Element::MmrTree(0, None)),
        (
            b"wrapped",
        ...
        assert_eq!(
            db.get_raw(
                SubtreePath::from(&[TEST_LEAF, b"counts"]),
                key,
                None,
                version
            )
...
fn delete_with_flag_handles_trees() {
    ...
    // Specialized data trees contain no reference participants and delete normally.
    db.insert(
        &[TEST_LEAF],
        b"mmr",
    ...
    .unwrap()
    .unwrap();
    db.delete(&[TEST_LEAF], b"mmr", None, None, grove_version)
        .unwrap()
        .unwrap();
    assert!(db
        .get_raw(SubtreePath::from(&[TEST_LEAF]), b"mmr", None, grove_version)
        .unwrap()
        .is_err());
}
...
fn verify_grovedb_reports_corrupt_provided_value_hashes() {
    ...
    merk.apply::<_, Vec<u8>>(
        &[(
            b"bad".to_vec(),
            MerkOp::PutWithProvidedValueHash(bytes, [7; 32], None, TreeFeatureType::BasicMerkNode),
        )],
        &[],
        None,
        grove_version,
...
fn caller_supplied_referrer_lists_are_discarded_on_insert() {
    ...
    db.insert(
        &[TEST_LEAF],
        b"planted",
        Element::ItemWithBackwardsReferences(b"x".to_vec(), vec![forged.clone()].into(), None),
        flag_on(),
        Some(&tx),
        grove_version,
    )
...
#[test]
fn automatic_delete_handles_specialized_descendants() {
    let grove_version = GroveVersion::latest();
    let db = make_test_grovedb(grove_version);
    db.insert(
        &[TEST_LEAF],
...
fn verify_grovedb_reports_forged_referrer_entries() {
    ...
    merk.apply::<_, Vec<u8>>(
        &[(
            b"forged".to_vec(),
            MerkOp::PutWithProvidedValueHash(bytes, combined, None, TreeFeatureType::BasicMerkNode),
        )],
        &[],
        None,
        grove_version,
...
/// A cascade must physically remove the referrers it deletes. Deleting
/// `value` rebalances the subtree and rewrites `ref`'s node in the shared
/// batch; the cascade's later delete of `ref` must supersede that rewrite
/// rather than being dropped by put-wins merging.
#[cfg(feature = "unsafe-dump-load")]
#[test]
fn cascade_removes_the_physical_referrer_record() {
    ...
    // The raw row is gone, not just unreachable.
    let tx = db.start_transaction();
    let storage = db
        .raw_storage()
        .get_immediate_storage_context((&[TEST_LEAF]).into(), &tx)
        .unwrap();
    let ghost = storage.get(b"ref").unwrap().unwrap();
    drop(storage);
    drop(tx);
    assert!(
        ghost.is_none(),
        "cascade left a physical referrer record behind"
    );

    // And a later unrelated insert cannot resurrect it into range results.
    db.insert(
        &[TEST_LEAF],
        b"fresh",
    ...
    let (results, _) = db
        .query_raw(
            &PathQuery::new_unsized(vec![TEST_LEAF.to_vec()], query),
            true,
            true,
            true,
    ...
    let keys: Vec<Vec<u8>> = results
        .into_iterator()
        .map(|r| match r {
            QueryResultElement::KeyElementPairResultItem((key, _)) => key,
            other => panic!("unexpected result shape: {other:?}"),
        })
        .collect();
...
/// Cascaded deletions honour the caller's sectioned-removal policy: the
/// callback sees the referrer's flags too, and its accounting decision
/// applies to the referrer's removed bytes.
#[test]
fn cascade_forwards_the_sectioned_removal_callback() {
    ...
    .unwrap()
    .unwrap();
    // The referrer lives in a separate subtree, with its own flags.
    db.insert(
        &[TEST_LEAF, b"refs"],
        b"ref",
        Element::BidirectionalReference(
    ...
    assert!(
        db.get_raw(
            SubtreePath::from(&[TEST_LEAF, b"refs"]),
            b"ref",
            None,
            grove_version
        )
        .unwrap()
        .is_err(),
        "the referrer must be cascaded away"
    );
    assert!(
        seen_flags.contains(&vec![1]) && seen_flags.contains(&vec![2]),
        "the callback must run for the target AND the cascaded referrer, saw {seen_flags:?}"
    );
}
...
fn stale_nonconsenting_registration_does_not_block_target_deletion() {
    ...
    .unwrap()
    .expect("a stale registration must not block deleting its former target");
    assert!(matches!(
        db.get(&[TEST_LEAF], b"value", None, grove_version).unwrap(),
        Err(Error::PathKeyNotFound(_))
    ));
}

/// Hand-built wrappers around backward-references elements (no constructor
/// produces them, deserialization rejects them) are refused by every insert
/// version: the GROVE_V4 router before either route, and the pre-V4 gate,
/// which checks the underlying element.
#[test]
fn wrapped_backward_references_elements_are_refused_on_insert() {
    let grove_version = GroveVersion::latest();
    let db = make_test_grovedb(grove_version);
    db.insert(
        &[TEST_LEAF],
    ...
    let err = db
        ...
        .unwrap()
        .expect_err("GROVE_V4 refuses a wrapped family element");
    assert!(matches!(err, Error::InvalidInput(_)), "{err:?}");

    let v3 = &grovedb_version::version::v3::GROVE_V3;
    assert_eq!(v3.protocol_version, 3);
    let err = db
        .insert(&[TEST_LEAF, b"ct"], b"w", wrapped, None, None, v3)
        .unwrap()
        .expect_err("pre-V4 refuses a wrapped family element");
...
/// An item's declared referrer capacity is enforced on the live path: a
/// full target refuses another reference, an update may not lower the
/// capacity below the registered referrers, and raising it admits more.
#[test]
fn declared_capacity_is_enforced_on_live_registration_and_updates() {
    let grove_version = GroveVersion::latest();
    let db = make_test_grovedb(grove_version);
    db.insert(
        &[TEST_LEAF],
    ...
    .unwrap()
    .expect("target declaring one referrer");
    db.insert(
        &[TEST_LEAF],
        b"r1",
    ...
    .unwrap()
    .expect("the first referrer fits");
    ...
    assert!(
        db.get_raw([TEST_LEAF].as_ref().into(), b"r2", None, grove_version)
            .unwrap()
            .is_err(),
        "a refused reference is not stored"
    );

    // Lowering the capacity below the registered referrer is refused …
    let err = db
        .insert(
            &[TEST_LEAF],
            b"target",
    ...
    assert_eq!(
        db.get(&[TEST_LEAF], b"r1", None, grove_version)
            .unwrap()
            .unwrap()
            .as_item_bytes()
            .unwrap(),
    ...
    // … raising it propagates like any update and admits the second.
    db.insert(
        &[TEST_LEAF],
        b"target",
        Element::new_item_allowing_bidirectional_references_with_capacity(b"w".to_vec(), 3),
        flag_on(),
        None,
        grove_version,
    )
    .unwrap()
    .expect("raising the capacity");
    db.insert(
        &[TEST_LEAF],
        b"r2",
        sibling_bidi(b"target", true),
    ...
    .unwrap()
    .expect("the second referrer now fits");
    for referrer in [b"r1".as_ref(), b"r2"] {
        assert_eq!(
            db.get(&[TEST_LEAF], referrer, None, grove_version)
                .unwrap()
                .unwrap()
                .as_item_bytes()
                .unwrap(),
    ...
    // Public reads strip the referrer list but keep the declaration.
    let stored = db
        .get_raw([TEST_LEAF].as_ref().into(), b"target", None, grove_version)
        .unwrap()
        .unwrap();
    assert_eq!(stored.max_incoming_references(), Some(3));
    assert!(stored.backward_references().unwrap().is_empty());
    // A third referrer still fits the raised capacity, a fourth does not:
    // the two registrations survived the update.
    db.insert(
        &[TEST_LEAF],
        b"r3",
        sibling_bidi(b"target", true),
    ...
    .unwrap()
    .expect("the third referrer fits a capacity of three");
    assert!(db
        .insert(
            &[TEST_LEAF],
            b"r4",
