...
fn declared_capacity_tightens_the_worst_case_estimate() {
    ...
    let tight = worst_case_estimate(family(1), batch_flag_on(), grove_version);
    let default = worst_case_estimate(
        family(grovedb_element::DEFAULT_BACKWARD_REFERENCES_CAPACITY),
        batch_flag_on(),
        grove_version,
    );
    let ceiling = worst_case_estimate(plain, batch_flag_on(), grove_version);
    assert!(
    ...
    let db = db_with_chain(grove_version);
    let actual = db
        .apply_batch(family(1), batch_flag_on(), None, grove_version)
        .cost_as_result()
    ...
    let tight_average = average_case_estimate(family(0), batch_flag_on(), grove_version);
