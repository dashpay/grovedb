...
mod tests {
    ...
    // =========================================================================
    // QueryItem variant coverage for non-Merk tree types (generate + verify)
    //
    ...
    #[test]
    fn prove_v1_bulk_append_tree_query_variants() {
        // Exercises 8 QueryItem range variants against a BulkAppendTree.
        // Covers generate.rs::query_items_to_range and
        // verify.rs::extract_range_from_query_items + expand_query_to_u64_positions.
        let grove_version = GroveVersion::latest();
        let db = make_empty_grovedb();
        ...
        for i in 0..6u8 {
            ...
            .unwrap()
            .expect("append to bulk append tree");
        }

        let be = |v: u64| -> Vec<u8> { v.to_be_bytes().to_vec() };

        // (variant_name, QueryItem, expected_result_count)
        let variants: Vec<(&str, QueryItem, usize)> = vec![
            // Range: positions 1..4 (exclusive end) → 1, 2, 3
            ("Range", QueryItem::Range(be(1)..be(4)), 3),
            // RangeFrom: positions 3.. → 3, 4, 5
            ...
            // RangeFull: all → 0, 1, 2, 3, 4, 5
            ("RangeFull", QueryItem::RangeFull(std::ops::RangeFull), 6),
            // RangeAfter: after 1 (exclusive) → 2, 3, 4, 5
            ("RangeAfter", QueryItem::RangeAfter(be(1)..), 4),
            // RangeAfterTo: after 0, before 4 → 1, 2, 3
            ("RangeAfterTo", QueryItem::RangeAfterTo(be(0)..be(4)), 3),
            // RangeAfterToInclusive: after 0, through 3 → 1, 2, 3
            (
        ...
        for (name, item, expected_count) in variants {
            let pq = make_subquery_path_query(b"bat", item);
            let count = prove_and_verify_v1(&db, &pq, grove_version);
            assert_eq!(
                count, expected_count,
                "BulkAppend variant {name}: expected {expected_count} results, got {count}"
            );
        }
    }
