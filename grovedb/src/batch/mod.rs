...
use merk::{
    ...
    CryptoHash, Merk, MerkOptions, MerkType, TreeFeatureType,
};
    ...
    S: StorageContext<'db>,
{
    ...
    fn follow_reference_get_value_hash<'a>(
        ...
        if let Some(op) = ops_by_qualified_paths.get(qualified_path) {
            ...
            let (reference_merk_wrapped, is_sum_tree) = self
                .merks
                .remove(reference_path)
                .map(|(merk, is_sum_tree)| {
                    (Ok(merk).wrap_with_cost(Default::default()), is_sum_tree)
                })
    ...
    S: StorageContext<'db>,
{
    fn insert(&mut self, op: &GroveDbOp, is_sum_tree: bool) -> CostResult<(), Error> {
        ...
        if !self.merks.contains_key(&inserted_path) {
            let (merk_wrapped, _) = (self.get_merk_fn)(&inserted_path, false);
    ...
    fn execute_ops_on_path(
        ...
        let (root_hash, root_key) = merk.root_hash_and_key().unwrap_add_cost(&mut cost);
        (root_hash, root_key, merk.sum())
            .wrap_with_cost(cost)
            .map(Ok)
    }
}
...
impl GroveDb {
        ...
        while let Some(ops_at_level) = ops_by_level_paths.remove(&current_level) {
            for (path, ops_at_path) in ops_at_level.into_iter() {
                if current_level == 0 {
                    ...
                    let (root_hash, calculated_root_key, merk_sum) = cost_return_on_error!(
                        &mut cost,
                        merk_tree_cache.execute_ops_on_path(
                            &path,
                            ops_at_path,
    ...
    fn apply_body<'db, S: StorageContext<'db>>(
        ...
        get_merk_fn: impl FnMut(&[Vec<u8>], bool) -> (CostResult<Merk<S>, Error>, bool),
    ) -> CostResult<(), Error> {
        let mut cost = OperationCost::default();
            ...
            Some(key) => {
                if new_merk {
                    ...
                    let parent_storage = self
                        ...
                        .unwrap_add_cost(&mut cost);
                    let element_result =
                        Element::get_from_storage(&parent_storage, key).unwrap_add_cost(&mut cost);
                    let element = if element_result.is_err() {
                        return (
                        ...
                        element_result.expect("confirmed not error")
                    };
                    if let Element::Tree(root_key, _) = element {
                        (
                        ...
                        .wrap_with_cost(OperationCost::default()), false)
                    }
                }
            }
            None => {
                if new_merk {
                    if is_sum_tree {
                        (
                            Ok(Merk::open_empty(storage, MerkType::BaseSumMerk))
                                .wrap_with_cost(cost),
                            is_sum_tree,
                        )
                    } else {
                    ...
                    (
                        Merk::open_base(storage, is_sum_tree)
                            .map_err(|_| {
    ...
    pub fn open_batch_merk_at_path<'a>(
        ...
        if new_merk {
            let merk_type = if path.is_empty() {
                if is_sum_tree {
                    MerkType::BaseSumMerk
                } else {
                    MerkType::BaseMerk
                }
            } else {
                if is_sum_tree {
                    MerkType::LayeredSumMerk
                } else {
            ...
            if let Some((last, base_path)) = path.split_last() {
                let parent_storage = self
                    ...
                    .unwrap_add_cost(&mut local_cost);
                let element_result =
                    Element::get_from_storage(&parent_storage, last).unwrap_add_cost(&mut local_cost);
                let element = if element_result.is_err() {
                    return (
                        Err(Error::InvalidPath(
                            "could not get key for parent of subtree for batch".to_owned(),
                ...
                if let Element::Tree(root_key, _) = element {
                    ...
                    .wrap_with_cost(local_cost), false)
                }
            } else {
                (
                    Merk::open_base(storage, is_sum_tree)
