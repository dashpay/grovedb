...
//
// Under `BatchApplyOptions::propagate_backward_references_when_unsure` (GROVE_V4+), a
...
// the apply path's hard budgets (an item DECLARES how many referrers it
// accepts, at most `MAX_BACKWARD_REFERENCES` = 256; 1 per reference;
// `MAX_REFERENCE_HOPS` = 10 per component):
//
// - A write of a backward-references ITEM carries its declared capacity,
//   and the apply path refuses the write unless every referrer carried
//   over from the displaced element fits it — so that write's fan-out is
//   bounded by the DECLARED capacity (plus the de-registration a displaced
//   reference owes), and an item declaring four referrers pays for four.
// - The estimator cannot see the STORED element any OTHER op displaces
//   (a plain payload, a reference, a delete), so those charge the
//   displaced-state bound at the protocol ceiling — a plain payload can
//   land on a registered family element whose bookkeeping the
//   preprocessor must perform.
// - WORST case charges the full bound — an item's referrer graph is at
//   most `capacity` chains of at most `MAX_REFERENCE_HOPS` nodes each,
//   every one rewritten (or cascaded away); a reference insertion
//   additionally touches its target, its old target, and its single
//   upstream chain.
...
impl BackwardReferencesFanOut {
    ...
    /// The worst-case fan-out of a write of a backward-references ITEM
    /// declaring `max_incoming` referrers. The write carries its own
    /// declaration, and the apply path refuses it unless every referrer
    /// carried over from the displaced element fits that capacity, so the
    /// propagation it can trigger is bounded by the DECLARED capacity
    /// rather than the protocol ceiling: at most `max_incoming` chains of
    /// `MAX_REFERENCE_HOPS` nodes. Displacing a bidirectional reference
    /// additionally removes its registration from its old target (one
    /// rewrite, one chain resolution).
    pub(in crate::batch) fn worst_item_with_capacity(max_incoming: u16) -> Self {
        let hops = crate::operations::get::MAX_REFERENCE_HOPS as u32;
        let chains = max_incoming as u32 * hops;
        Self {
            rewrites: chains + 1,
            resolution_loads: chains + hops,
            propagations: chains + 1,
            registration_added_bytes: 0,
        }
    }
    ...
    /// The average-case fan-out of a write of a backward-references ITEM
    /// declaring `max_incoming` referrers: the typical item shape, never
    /// more than the declared worst case (an item declaring no referrers
    /// can only owe the de-registration of a displaced reference).
    pub(in crate::batch) fn average_item_with_capacity(max_incoming: u16) -> Self {
        let worst = Self::worst_item_with_capacity(max_incoming);
        Self {
            rewrites: BACKWARD_REFERENCES_AVERAGE_ITEM_FAN_OUT.min(worst.rewrites),
            resolution_loads: BACKWARD_REFERENCES_AVERAGE_ITEM_FAN_OUT.min(worst.resolution_loads),
            propagations: 1.min(worst.propagations),
            registration_added_bytes: 0,
        }
    }
