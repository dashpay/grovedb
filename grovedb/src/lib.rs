...
impl GroveDb {
    ...
    fn propagate_multiple_path_changes_on_transaction<'a, 'db>(
        &self,
        paths: Vec<Vec<&[u8]>>,
        ...
        let mut path_map: IntMap<usize, Vec<Vec<&[u8]>>> = IntMap::default();
        ...
        paths.into_iter().for_each(|p| {
            let p_iter = p;
            let len = p_iter.len();
            if let Some(mut path_map_level) = path_map.get_mut(&len) {
                path_map_level.push(p_iter);
            } else {
                path_map.insert(len, vec![p_iter]);
                if current_last_level < len {
                    current_last_level = len;
                }
            }
        });

        let mut upper_tree_cache: HashMap<Vec<&[u8]>, Merk<PrefixedRocksDbTransactionContext>> =
            HashMap::new();

        // We will update up the tree
        while let Some(paths_at_level) = path_map.remove(&current_last_level) {
            current_last_level -= 1;
            for path in paths_at_level.into_iter() {
                let (subtree, mut path_iter) = if let Some(merk) =
                    upper_tree_cache.remove(path.as_slice())
                {
                    (merk, path.clone().into_iter())
                } else {
                    let path_iter = path.clone().into_iter();
                    let subtree_storage = self
                        .db
                        .get_transactional_storage_context(path_iter.clone(), transaction);
                    let newly_opened_subtree = cost_return_on_error!(
                        &mut cost,
                        Merk::open(subtree_storage)
                            .map_err(|_| Error::CorruptedData("cannot open a subtree".to_owned()))
                    );
                    (newly_opened_subtree, path_iter)
                };
                ...
                if let Some(mut path_map_level) = path_map.get_mut(&current_last_level) {
                    path_map_level.push(path);
                } else {
                    path_map.insert(current_last_level, vec![path]);
                }
            }
        }
