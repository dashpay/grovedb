...
///
/// A position is written at most once: a later write of a position the
/// plan already writes folds into the earlier slot (last element and end
/// hash win, the primary marker is kept). Planners read the pre-plan store,
/// never their own earlier writes, so folding changes no planning decision
/// — it only spares drivers an intermediate overwrite (and, for the primary
/// position, keeps the caller's insert options on the write that becomes
/// final: propagation's lazy referrer-list cleanup can land on the very
/// reference being inserted).
...
impl Plan {
    fn write(
        ...
        self.push_write(path, key, element, end_hash, false);
    }
    ...
    fn write_primary(
        ...
        self.push_write(path, key, element, end_hash, true);
    }
    ...
    fn push_write(
        ...
        let existing = self.mutations.iter_mut().find(|mutation| {
            matches!(
                mutation,
                DerivedMutation::Write {
                    ...
                    key: existing_key,
                    ..
                } if *existing_path == path && *existing_key == key
            )
        });
        match existing {
            Some(DerivedMutation::Write {
                element: existing_element,
                end_hash: existing_end_hash,
                is_primary: existing_is_primary,
                ..
            }) => {
                *existing_element = element;
                *existing_end_hash = end_hash;
                *existing_is_primary |= is_primary;
            }
            _ => self.mutations.push(DerivedMutation::Write {
                path,
                key,
                element,
                end_hash,
...
fn with_registration(target: Element, entry: BackwardReference) -> Result<Element, Error> {
        ...
        ))?;
    {
            ...
            .find(|r| r.inverted_reference == entry.inverted_reference)
        {
            *existing = entry;
            ...
            // The target declares how many referrers it accepts (an item's
            // `max_incoming`, one for a bidirectional reference); a full
            // list refuses the registration rather than growing past what
            // the target's owner agreed to pay for on updates.
            if refs.len() >= capacity as usize {
                return Err(Error::BidirectionalReferenceRule(format!(
                    "the target accepts at most {capacity} incoming backward references and is \
                     full"
                )));
            }
            refs.push(entry);
    ...
    target.validate_backward_references_limits().map_err(|e| {
        Error::BidirectionalReferenceRule(format!("backward references budget exceeded: {e}"))
    })?;
    Ok(target)
}

/// A family item written over a position that carries referrers keeps
/// that referrer list (drivers carry it over before planning); the list
/// must fit the capacity the NEW element declares. Lowering an item's
/// capacity below its registered referrers is refused, raising it is
/// always accepted.
pub(crate) fn check_carried_referrers_fit(new: &Element) -> Result<(), Error> {
    new.validate_backward_references_limits().map_err(|_| {
        let registered = new.backward_references().map(|r| r.len()).unwrap_or(0);
        let capacity = new.max_incoming_references().unwrap_or(0);
        Error::BidirectionalReferenceRule(format!(
            "the element declares a capacity of {capacity} incoming backward references but \
             {registered} referrers are registered on it; a capacity cannot be lowered below \
             the registered referrers"
        ))
    })
}
...
pub(crate) fn plan_element_update(
    ...
    match (old, new) {
        ...
        (
            ...
            // registration on the old target. The reference's own referrer
            // (carried over) must fit the item's declared capacity.
            cost_return_on_error_no_add!(cost, check_carried_referrers_fit(&new));
            let new_logical_hash = cost_return_on_error!(
                &mut cost,
                new.logical_value_hash(store.version()).map_err(Error::from)
...
pub(crate) fn plan_cascade(
    ...
    while let Some((current_path, current_key, current_element, first)) = queue.pop_front() {
        ...
        for backward_ref in backward_references {
            ...
            // Consent is only required from referrers that still exist: a
            // stale registration left behind by an unflagged write must not
            // block deleting its former target.
...
mod tests {
    ...
    /// Retargeting a reference whose own referrer list carries a dangling
    /// entry: the primary write and propagation's lazy cleanup of that
    /// entry land on the same position, so the plan folds them into ONE
    /// primary write carrying the cleaned element (the caller's insert
    /// options must apply to the write that becomes final).
    #[test]
    fn lazy_cleanup_on_the_inserted_reference_folds_into_the_primary_write() {
        let store = MapStore::new();
        let registration = |referrer: &[u8]| BackwardReference {
        ...
        store.put(
            &[LEAF],
            b"new",
            Element::new_item_allowing_bidirectional_references(b"n".to_vec()),
        );
        // `gone` was removed by an unflagged write: its registration on the
        // reference is stale.
        let mut existing = sibling_bidi(b"old", true);
        existing.backward_references = vec![registration(b"gone")];
        store.put(
            &[LEAF],
            b"ref",
            Element::BidirectionalReference(existing, None),
        );

        let plan =
            plan_reference_insertion(&store, &leaf(), b"ref", sibling_bidi(b"new", true), None)
                .unwrap()
                .unwrap()
                .expect("a retarget is not an identical edge");

        let ref_writes: Vec<_> = plan
            .mutations
            .iter()
            .filter(
                |mutation| matches!(mutation, DerivedMutation::Write { key, .. } if key == b"ref"),
            )
            .collect();
        assert_eq!(ref_writes.len(), 1, "one write per position: {plan:?}");
        let DerivedMutation::Write {
            element,
            end_hash,
            is_primary,
            ...
            unreachable!("filtered on writes");
        };
        assert!(is_primary, "the folded write keeps the primary marker");
        assert!(end_hash.is_some(), "and the reference's end hash");
        let Element::BidirectionalReference(written, _) = element else {
            panic!("the reference itself is written");
        };
        assert!(
            written.backward_references.is_empty(),
            "the dangling registration was cleaned on the same write"
        );
        assert_eq!(
            written.forward_reference_path,
            ReferencePathType::SiblingReference(b"new".to_vec())
        );
        // The other two writes: registration on `new`, de-registration on
        // `old`.
        assert_eq!(plan.mutations.len(), 3, "{plan:?}");
    }

    /// A target that has reached its declared capacity refuses another
    /// registration.
    #[test]
    fn registration_refuses_a_full_target() {
        let store = MapStore::new();
        let registration = |referrer: &[u8]| BackwardReference {
            inverted_reference: ReferencePathType::SiblingReference(referrer.to_vec()),
            cascade_on_update: true,
        ...
        store.put(
            ...
            Element::ItemWithBackwardsReferences(
                b"v".to_vec(),
                grovedb_element::BackwardReferences::new(1, vec![registration(b"first")]),
                None,
            ),
        );
        store.put(
        ...
        let err = plan_reference_insertion(
            &store,
            &leaf(),
            b"second",
        ...
        assert!(
            matches!(&err, Error::BidirectionalReferenceRule(msg) if msg.contains("full")),
            "{err:?}"
        );

        // Re-registering the SAME referrer (an edge update) is an upsert,
        // not a new registration, and still fits.
        assert!(plan_reference_insertion(
            &store,
            &leaf(),
            b"first",
            sibling_bidi(b"target", false),
            None
        )
        .unwrap()
        .is_ok());
    ...
    /// Updating an item may raise its capacity freely but may not lower it
    /// below the referrers already registered on it.
    #[test]
    fn update_cannot_lower_capacity_below_registered_referrers() {
        let store = MapStore::new();
        let registration = |referrer: &[u8]| BackwardReference {
            inverted_reference: ReferencePathType::SiblingReference(referrer.to_vec()),
            cascade_on_update: true,
        };
        let registered = vec![registration(b"r1"), registration(b"r2")];
        for referrer in [b"r1".as_ref(), b"r2"] {
            store.put(
                &[LEAF],
                referrer,
                Element::BidirectionalReference(sibling_bidi(b"target", true), None),
        ...
        let old = Element::ItemWithBackwardsReferences(
            b"v".to_vec(),
            grovedb_element::BackwardReferences::new(4, registered.clone()),
            None,
        );
        store.put(&[LEAF], b"target", old.clone());

        // Drivers carry the stored list onto the new element before
        // planning; the capacity is whatever the caller declared.
        let carried = |capacity: u16| {
            Element::ItemWithBackwardsReferences(
                b"w".to_vec(),
                grovedb_element::BackwardReferences::new(capacity, registered.clone()),
                None,
            )
        };
        let err = plan_element_update(&store, &leaf(), b"target", old.clone(), Some(carried(1)))
            .unwrap()
            .expect_err("two referrers do not fit a capacity of one");
        assert!(
            matches!(&err, Error::BidirectionalReferenceRule(msg) if msg.contains("lowered")),
            "{err:?}"
        );
        for capacity in [2u16, 8] {
            let plan = plan_element_update(
                &store,
                &leaf(),
                b"target",
                old.clone(),
                Some(carried(capacity)),
            )
            .unwrap()
            .expect("the registered referrers fit");
            assert_eq!(
                plan.mutations.len(),
                2,
                "both referrers are rewritten: {plan:?}"
            );
        }
    }
